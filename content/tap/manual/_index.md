...
Navigate between tracks by swiping left or right. To select a track, tap on it.  
A long-press opens the track’s context menu where you can add tracks, delete tracks, configure the standard note length for the step sequencers, go [Home](#3-1-home) or go to [Encoders](#3-6-encoders-view).
...
Each note you add will have the velocity set in the side panel (see [Footer Bar](#3-2-2-footer-bar) button one)
...
2. The looped section is indicated by the band on top of the step sequencer and 2 lines at the start and end of the loop all in color of the track.
...
8. Indicates if this clip is playing: solid play symbol. If another clip on this track is playing, the play symbol is outlined only. If noclip in this track is playing the play symbol is displayed with a dash.
...
{{ image_sets(path="content/tap/manual/Step Seq Context Menu.jpg", format="auto", op="fit_width", quality=75, alt="Tap Step Sequencer Context Menu", caption='Lil Green Helper menu.', imgset_class="imgset-half") }}
...
2. & 3. Move buttons: Move up and down the octave in instruments, up and down the drums page for drums.
...
7. & 9. Move Loop Start/End & Loop: Moves the start/end of the loop and the whole loop (this means no change in loop lenght) to where the lil green rectangle sits.
8. & 10 Move Loop Start/End: Only moves the start/end of the loop to where the lil green rectangle sits. This will change the lenght of the loop.
11. Start selected Clip, will start the selected clip (which is always the one you are seeing in the step sequencer)-
...
15. & 16. Start/End Marker: Moves the start/end marker to the lil green rectangle.
