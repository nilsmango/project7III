+++
title = "7III Tap: Ableton Live Controller for iPhone & iPad"
description = "Control Ableton Live from iPhone or iPad with 7III Tap. Browse samples, edit audio clips, sequence, mix and play expressive MPE pads via USB, Wi-Fi or Bluetooth MIDI."
[extra]
date = 2024-03-15
updated = 2026-09-25
share = true
seo_title = "Ableton Live Controller for iPhone & iPad | 7III Tap"
featured_image = "7III Tap Ableton MIDI Controller Step Sequencer.jpg"
featured_image_alt = "7III Tap Ableton Live Controller on an iPad"
+++
<a href="/tap/manual" class="btn" id="greenButton">User Manual</a> <a href="/tap/videos" class="btn" id="greenButton">Videos</a> <a href="/tap/support" class="btn" id="greenButton">Support</a> <a href="/tap/history" class="btn" id="greenButton">Version History</a> <a href="/tap/best-ableton-live-controller" class="btn" id="greenButton">Controllers for Ableton Live</a> <a href="https://project7iii.com/tap/Tap.zip" class="btn" id="purpleButton">↓ MIDI Remote Script 2.1.1</a>

>Last Update: **{{ date_updated() }}**  
>Tap 2.1(.1) has landed: browse samples, edit audio clips, play expressive MPE pads and Note Repeat, and explore deeper Simpler controls. Now with Bluetooth MIDI too!  
👉 [See everything new in version 2.1](/tap/history/#version-2-1), then [try Tap for free on the App Store](https://apps.apple.com/app/7iii-tap-live-midi-controller/id6499097273).


{{ image_sets(path="content/tap/7III Tap Ableton MIDI Controller Step Sequencer.jpg", format="auto", op="fit_width", quality=100, loading="eager", fetchpriority="high", alt="7III Tap controller running on an iPad, showing the melodic step sequencer", caption='7III Tap melodic step sequencer with device parameters') }}

7III Tap turns your **iPhone or iPad into a hands-on Ableton Live controller**. Browse samples, edit audio clips, launch scenes, mix tracks, sequence drums and melodies, record or draw automation, and control devices from a fast, responsive touchscreen built for performance, production, and sound design.

## 7III Tap at a Glance

<table class="facts-table">
  <tbody>
    <tr><th scope="row">Mobile devices</th><td>iPhone and iPad</td></tr>
    <tr><th scope="row">Computer support</th><td>macOS and Windows</td></tr>
    <tr><th scope="row">Connections</th><td>USB, Wi-Fi and Bluetooth MIDI on macOS; wired MIDI, rtpMIDI and experimental Bluetooth MIDI on Windows</td></tr>
    <tr><th scope="row">Main controls</th><td>Devices, MIDI and audio clips, mixer, step sequencers, automation, MPE and MIDI CC</td></tr>
    <tr><th scope="row">Setup</th><td>Included Ableton Live MIDI Remote Script; no server or Max for Live device</td></tr>
    <tr><th scope="row">Price</th><td>Free 15-minute daily trial; one-time Premium unlock; no subscription</td></tr>
  </tbody>
</table>

## Why an Ableton Live Controller App?
Why turn two knobs when you have ten fingers and could tweak up to 8 macros at a time and still have two fingers left for mixing? Hardware controllers have their charm, but at project7III we focus on the future. A touchscreen is simply the most flexible and precise tool for adjusting parameters, launching clips, sequencing, editing automation, and mixing.  
That's why we created Tap: a pocket-sized Ableton Live controller built for real performance, deep control, and fast creative work. If you have a few bucks to spare and an iOS device, you're ready to upgrade your workflow.  
  
Here are some of the reasons why you should consider Tap:
- **Works on both iPhone and iPad**  
- **Flexible connections** via USB, Wi-Fi, or Bluetooth MIDI on macOS, plus wired MIDI, rtpMIDI, and experimental Bluetooth MIDI on Windows
- **Designed for live performance**, production, sound design, and spontaneous jams
- **Three powerful views**: Device, Clips, Mixer
- **Audio clip editing and sample browsing**: find and load sounds, zoom into waveforms, move Warp markers, shape loops, and adjust gain and pitch
- **Powerful Automation Editing**: record or draw curves, adjust individual points, fine-tune transitions, and use ready-made envelopes
- **Expressive MPE Pads and Keys** with per-note Pitch Bend and Pressure or Slide
- **Note Repeat** for repeating notes and drum patterns
- **Follow Actions** for evolving clips and performances
- **Companion**, a melodic and rhythmic partner that plays along with you and turns your clips into musical variations
- **Rhythm Generator** for quickly creating new grooves and ideas
- **Flin generative sequencer** with 16 independent polymetric note streams written directly into Live clips
- **Native Simpler integration** with sample loading, waveform visualization, slicing, and dedicated controls
- **Macro Variations and Randomization** for instant sound exploration
- **Searchable Browser** for quickly finding and adding devices, effects, presets, and samples
- **Universal MIDI CC mode** for controlling any instrument or app
- **Adaptive layouts** for portrait and landscape on iPad
- **Test Tap for free** before you buy it. No subscription. One-time unlock

### Device View
- Different MIDI keyboards & pads  
- Drum and melodic step sequencer  
- Devices & banks  
- Curated encoder controls for Ableton Live instruments and effects
- Track controls for mixing without leaving Device View
- 8 encoders per bank
- MIDI modulation tools and powerful note editing  
- Audio clip editing with zoomable waveforms, Warp markers, loop controls, gain, pitch, cropping, and conversion tools
- Sample browsing and search to find and load sounds for audio clips
- MPE, Note Repeat, and 16-pitch or 64-pad layouts for expressive playing
- Powerful Automation Editing, Macro Variations, and generative MIDI tools  

Designed for quick sound design, live modulation, performance, and awesome sequencing.

### Audio Clips and Samples
Browse and search for sounds in Live, load samples, and shape your audio clips from Tap. Zoom into the waveform, move Warp markers, set start and loop points, and adjust gain, transpose, and detune. Crop a clip or use Live’s conversion tools to move from audio to Simpler, a Drum Pad, or MIDI where supported by your Live edition.

See the [audio clip guide](/tap/manual/#3-3-6-audio-clips) for controls and gestures.

### Clips View
- Launch, record, append (!), copy & duplicate clips  
- Trigger and duplicate scenes  
- Use Follow Actions for evolving clips and performances  
- Access devices and encoders without switching to another screen  

### Mixer View
- Volume, sends, pan, solo, and mute layout  
- 10 Finger mixing!
- Integrated clips panel for mixing and launching clips at the same time

{{ image_sets(path="content/tap/Mixing Ableton Live with Tap.jpg", format="auto", op="fit_width", quality=100, loading="lazy", alt="Mixing and launching clips in Ableton Live with 7III Tap", caption='7III Tap Mixer View with clips panel') }}


### Custom Encoders
- Control any MIDI-compatible device or software with fully customizable encoders.  
- Perfect for synths, drum machines, external apps, and master effects parameters.

### Connectivity
- **Direct USB** (macOS only)
- **Bluetooth MIDI**
- **Wi-Fi**
- **Wired USB MIDI interface setup**
- Includes a custom Ableton Live Remote Script for deep integration
- No server or Max for Live device required. Simply download the MIDI Remote Script and put it in the correct folder. More information [here](/tap/manual/#2-1-add-the-tap-midi-remote-script).

## App Store
Download Tap on the App Store and test it for free for 15 minutes per day!
{{ appstore_button(tagline="Let's jam!", url="https://apps.apple.com/app/7iii-tap-live-midi-controller/id6499097273") }}

### Reviews
🏆 4.7/5 on the App Store (April 2026)

  <div class="reviews-container">
    <article class="review-card">
      <div class="review-header">
        <div class="review-stars">★★★★★</div>
        <div class="review-title">Ableton Live Control der genialen Art</div>
        <div class="review-meta">11. Sept. 2024 · Test-the-East</div>
      </div>
      <p class="review-text">
        Ich bin jetzt echt baff. TAP ist irgendwie total großartig durchdacht und nutzt die capture MIDI Funktion von Ableton Live gnadenlos gut aus… weiter bin ich noch nicht aber random drums, synths und effekte laden ist sowas von Jam Like! Fantastisch, was kommt denn da noch von Entwickler? Ich hoffe das ist nur der Anfang. 7.99€ Lächerlich, Kauft Leute, jeden Cent wert!
      </p>
    </article>
    <article class="review-card">
      <div class="review-header">
        <div class="review-stars">★★★★★</div>
        <div class="review-title">Not all app midi controllers are built the same!</div>
        <div class="review-meta">24. Sept. 2024 · JumperCollins</div>
      </div>
      <p class="review-text">
        Was pleasantly surprised how well this has been laid out and works. Gives loads of control. I actually stopped using my Push 2 for now as 7III TAP inspires more. This is what Touchable Pro should have been. For the price a no brainer. Go get!
      </p>
    </article>
    <article class="review-card">
      <div class="review-header">
        <div class="review-stars">★★★★★</div>
        <div class="review-title">Excelente controlador para Ableton</div>
        <div class="review-meta">2. Dez. 2024 · Press0data</div>
      </div>
      <p class="review-text">
        Un buen y económico recurso para producir y para usar en performances en vivo. Lo usé en Windows con el iPad y luego de la configuración inicial, funcionó sin problemas. Recomendado.
      </p>
    </article>
    <article class="review-card">
      <div class="review-header">
        <div class="review-stars">★★★★★</div>
        <div class="review-title">Beste Live Controller App</div>
        <div class="review-meta">4. Feb. 2025 · Mr. Reen</div>
      </div>
      <p class="review-text">
        Optisch 1A, auch die Integration der Rack Macros ist super.
      </p>
    </article>
    <article class="review-card">
      <div class="review-header">
        <div class="review-stars">★★★★★</div>
        <div class="review-title">Feels like a native app!</div>
        <div class="review-meta">14. Apr. 2025 · Kareemyusa</div>
      </div>
      <p class="review-text">
        Tap works so well with Ableton it feels like it was made as an official app. The design language matches Ableton so it's easy to get started quickly compared to other apps. I had a question and the owner replied to me in less than a day, which I think is amazing. I will update if I find any issues but this is a 5 star app for me!
      </p>
    </article>
    <article class="review-card">
      <div class="review-header">
        <div class="review-stars">★★★★★</div>
        <div class="review-title">Lifechanging</div>
        <div class="review-meta">12. Apr. 2025 · Spanish97</div>
      </div>
      <p class="review-text">
        This app literally changed my life in Live. It's super stable and fluid. Super useful for live session, mixing and creative process. It works very well for controlling your midi instrument and writing automations. That's all you need to give much more live feeling to your tracks. 100% worth it 👌
      </p>
    </article>
    <article class="review-card">
      <div class="review-header">
        <div class="review-stars">★★★★★</div>
        <div class="review-title">Excellent controller for ableton</div>
        <div class="review-meta">23. Nov. 2025 · Nizola Gonda</div>
      </div>
      <p class="review-text">
          Love this. Really nice way to control Ableton via my iPhone. Would love to see device shortcuts to quickly access fav instruments and also some preset morph would be cool.
      </p>
    </article>
    <article class="review-card">
      <div class="review-header">
        <div class="review-stars">★★★★★</div>
        <div class="review-title">This is fantastic app</div>
        <div class="review-meta">7. Jan. 2026 · kahunaoplomo</div>
      </div>
      <p class="review-text">
        This is what Ableton Note should be. App gives you a lot of control over DAW and makes it lightning fast to sketch ideas and work on them. Long Live The TAP!
      </p>
    </article>
  </div>

## Roadmap  
Things we have planned for Tap:
- More special dials, even deeper Ableton Live instruments and effect integration (already started with algoritms for Operator)
- More creative sequencers

{{ image_sets(path="content/tap/playing tap.jpg", format="auto", op="fit_width", quality=100, loading="lazy", alt="Playing Ableton Live with 7III Tap on an iPad", caption='Playing MIDI notes in Ableton Live with Tap') }}

>If you'd like to stay updated on Tap's development and contribute to new feature implementations, subscribe to our **7III Tap newsletter**.
><iframe src="https://embeds.beehiiv.com/ca2f2e50-b638-498a-a4b4-8e1b29f0cc5e?slim=true" data-test-id="beehiiv-embed" height="52" frameborder="0" scrolling="no" style="margin: 0; border-radius: 0px !important; background-color: transparent;"></iframe>

## FAQ

### Does Tap work with Ableton Live on Mac and Windows?
Yes. macOS supports direct USB, Wi-Fi, and Bluetooth MIDI. Windows supports wired MIDI interfaces, an experimental direct USB MIDI host bridge, Wi-Fi through rtpMIDI, and experimental Bluetooth MIDI through Windows MIDI Services. See the [connection guide in the manual](/tap/manual/#2-2-connect-your-iphone-or-ipad).

### Can Tap replace a hardware controller like Push?
For many workflows and most musicians, yes.  
Tap includes clip launching, device editing, mixing and a powerful step sequencer in a mobile form factor.

### Does Tap also work on iPhone?
Yes. Tap is one of the few fully featured iPhone Ableton Live controllers. Basically a Push the size of an iPhone.  

### Does Tap work on Android?
Not yet, sorry.

### Do I need additional software?
You only need to install the Tap MIDI Remote Script once. After that, Tap connects automatically.

### Does Tap support MPE or Aftertouch?
Yes. Pads and Keys support MPE with per-note Pitch Bend and Pressure or Slide. Standard velocity, modulation and pitch bend are supported too.

### Will Tap make my music sound better?
Unfortunately not. But you will be able to make more crappy music in less time, and gain the power to entertain (or terrify) crowds for hours, using only your laptop and your iPad.

### How does Tap compare to other Ableton Live Controllers?
Thank you for asking, we made a comparison just for you: [Compare Ableton Live Controllers](/tap/best-ableton-live-controller)

### Seven wtf... Tap?
**7III Tap** <span class="opacity-60">/sĕv′ən thrē tăp/</span>

**noun**  
The freshest Ableton Live controller on planet earth, made by project7III (project seven three).

### Is this a dream or reality?
No idea. But Tap is 100% the Ableton Live controller you've been dreaming about.   
Test it now: [Download 7III Tap](https://apps.apple.com/app/7iii-tap-live-midi-controller/id6499097273)

## Tap in the Media
Tap has already been featured by the music tech press:

### Listed by Ableton as an iOS controller for Live
Ableton includes 7III Tap in its official guide to controlling Live from an iOS device, highlighting its dedicated device, step sequencer, MIDI pad, mixer and clip views.

Source: [Ableton Help Center](https://help.ableton.com/hc/en-us/articles/209071989-Apps-for-controlling-Live-with-an-iOS-or-Android-device)

### Tap turns your iPhone into an Ableton Live controller — Create Digital Music  
> Tap turns your iPhone into an Ableton Live controller, taking some of the best ideas from Push and Ableton Note and adapting them to a mobile touchscreen.  

Source: [Create Digital Music](https://cdm.link/tap-turns-your-iphone-into-an-ableton-live-controller/)  


### Tap brings Ableton Live control to your iPhone — GEARNEWS
> Tap turns the iPhone into a versatile controller for Ableton Live, offering clip triggering, mixing and device control from your pocket.  

Source (German): [GEARNEWS](https://www.gearnews.de/tap-ableton-live-app-controller/) 

<br>
<br>

<div class="footnote-definition"><p>Tap legal stuff: <a href="/tap/terms-and-conditions">terms & conditions</a> & <a href="/tap/privacy-policy">privacy policy</a><br>Ableton Live is a trademark of Ableton AG, registered in the United States and other countries.
<br>iPhone and iPad are trademarks of Apple Inc., registered in the United States and other countries.</p></div>

<script type="application/ld+json">
{
  "@context": "https://schema.org",
  "@type": "MobileApplication",
  "name": "7III Tap: Live MIDI Controller",
  "url": "https://project7iii.com/tap/",
  "description": "Control Ableton Live from iPhone or iPad with 7III Tap. Browse samples, edit audio clips, sequence, mix and play expressive MPE pads via USB, Wi-Fi or Bluetooth MIDI.",
  "image": "https://project7iii.com/tap/7III%20Tap%20Ableton%20MIDI%20Controller%20Step%20Sequencer.jpg",
  "downloadUrl": "https://apps.apple.com/app/7iii-tap-live-midi-controller/id6499097273",
  "applicationCategory": "MultimediaApplication",
  "applicationSubCategory": "Ableton Live MIDI controller",
  "operatingSystem": "iOS, iPadOS",
  "softwareVersion": "2.1",
  "isAccessibleForFree": true,
  "offers": {
    "@type": "Offer",
    "price": 0,
    "priceCurrency": "USD",
    "description": "Free daily trial with a one-time Premium in-app purchase"
  },
  "author": {
    "@type": "Person",
    "name": "Simon Lang"
  },
  "publisher": {
    "@type": "Organization",
    "name": "project7III",
    "url": "https://project7iii.com/"
  },
  "review": {
    "@type": "Review",
    "author": {
      "@type": "Person",
      "name": "Kareemyusa"
    },
    "datePublished": "2025-04-14",
    "reviewRating": {
      "@type": "Rating",
      "ratingValue": 5,
      "bestRating": 5
    },
    "reviewBody": "Tap works so well with Ableton it feels like it was made as an official app."
  }
}
</script>
